...
class TaskAdapter(
    ...
    private fun progressFor(task: TreeTask): String? = when (task) {
        ...
        is HdhWeeklyTask -> {
            ...
            // Та сама логіка що і в HdhWeeklyTask.isCompleted() — від дати відкриття стадії
            ...
            val level = runCatching { TreeLevel.valueOf(saved) }.getOrDefault(TreeLevel.INDIVIDUAL)
            val stageOpenDate = tree.stageDateForLevel(level) ?: LocalDate.now()
            val stageWeekStart = stageOpenDate.with(java.time.DayOfWeek.MONDAY)

            var q = 0
            for (w in 0 until task.weeksRequired) {
                val weekStart = stageWeekStart.plusWeeks(w.toLong())
